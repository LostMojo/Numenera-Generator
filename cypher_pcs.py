...
class CypherPC:
    ...
    def __init__(self, name, tier=1, descriptor="adjective", char_type="noun", focus="verbs", skills={},
        ...
        if int_edge in range(-3, 20):  # ensure edge is an int between -3 and 20
            self._int_edge = int_edge
        else: raise ValueError("PC int edge must be an int between -3 and 20")
    ...
    def edge_list(self): return [self.might_edge(), self.speed_edge(), self.int_edge()]
    ...
    def assign_stat_point(self, pool):
        ...
        self.max_pools()[i] = self.max_pools()[i]+1         #make all the pools a [-, -, -] life will be easier
    ...
    def heal_full(self):
        self._current_pools = self.max_pools()
